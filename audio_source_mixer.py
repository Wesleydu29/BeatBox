...
class AudioSourceMixer(ThreadSource):
    ...
    def __init__(self, output_stream, *args, all_wav_samples, bpm, sample_rate, nb_steps, on_current_step_changed, min_bpm, **kwargs):
        ThreadSource.__init__(self, output_stream, *args, **kwargs)

        self.tracks = []

        for i in range(0, len(all_wav_samples)):
            track = AudioSourceTrack(output_stream, wav_samples=all_wav_samples[i], bpm=bpm, sample_rate=sample_rate, min_bpm=min_bpm)
