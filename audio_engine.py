...
class AudioEngine:
    ...
    def create_mixer(self, all_wav_samples, bpm, nb_steps, on_current_step_changed, min_bpm):
        source_mixer = AudioSourceMixer(self.output_stream, all_wav_samples=all_wav_samples, sample_rate =self.SAMPLE_RATE, bpm=bpm, nb_steps=nb_steps, on_current_step_changed=on_current_step_changed, min_bpm=min_bpm)
        source_mixer.start()
        return source_mixer